	...
	if(watching == 1)
		if(get_dist(src, T) >= 8)
			if(star_form == 1)
				...
				src << "\red This is too far to blink! I need to use my Teleportation ability to get there"
				return
...
/mob/living/simple_animal/jirachi/verb/energyblast(mob/living/carbon/human/M as mob in oview())
	...
	if(Q>=30)
		...
		if(M.species.flags & IS_SYNTHETIC || M_PSY_RESIST in M.mutations)
		...
		if(watching == 1 && star_form != 1)
			src << "\red I can't use any of my powers, while my mind is not in my body!"
			return
					...
					for(var/datum/organ/external/O in H.organs)
						...
						if(!O.wounds.len)
							...
							if(O.status & ORGAN_CUT_AWAY || O.status & ORGAN_ATTACHABLE || O.status & ORGAN_DESTROYED)
								...
								H << "\blue You start feeling your severed [O.name] growing back"
...
//Consciousness Imposition


/mob/living/simple_animal/jirachi/verb/trueeye()
	set category = "Jirachi"
	set name = "Consciousness Imposition"
	set desc = "Watch with other's eyes"
	...
	var/mob/target
	if(src.watching != 1)
		target = input ("Who do you want to project your mind to?") as mob in creatures
	...
	if (target)
		if(star_form != 1)
			for(var/mob/S in viewers(target, null))
				if (S.client && !(S.blinded) && S != target)
					S << "\red [target]'s eyes starts to glow with the faint light..."
			target << "\red Your eyes feels strange"
		remoteview_target = target
		watching = 1
		reset_view(target)
	else
		reset_view(0)
		src.remoteview_target = null
		src.watching = 0
...
/mob/living/simple_animal/jirachi/verb/showa()
	...
	abilities += "Forcewall"
	abilities += "Heal"
	abilities += "Consciousness Imposition"
	...
	switch(A)
		...
		if("Consciousness Imposition")
			src << ""
			src <<"Consciousness Impostion - allows Jirachi to watch with other's eyes and hear with other's ears. Star Form upgrade: no visual effects on target. Allows Jirachi to use Blink for twice of the cost"
